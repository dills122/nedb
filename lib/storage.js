 ...
 * It's essentially fs and crash safe write and read functions
 */
...
function mkdirpLegacy(dir, callback) {
  fs.mkdir(dir, function (err) {
    if (!err || err.code === 'EEXIST') { return callback(null); }
    if (err.code !== 'ENOENT') { return callback(err); }

    var parent = path.dirname(dir);
    if (parent === dir) { return callback(err); }

    mkdirpLegacy(parent, function (parentErr) {
      if (parentErr) { return callback(parentErr); }
      fs.mkdir(dir, function (mkdirErr) {
        if (!mkdirErr || mkdirErr.code === 'EEXIST') { return callback(null); }
...
function mkdirp(dir, callback) {
  ...
  try {
    fs.mkdir(dir, { recursive: true }, function (err) {
      ...
      if (err instanceof TypeError || err.code === 'ERR_INVALID_ARG_TYPE' || err.code === 'ERR_INVALID_ARG_VALUE') {
        return mkdirpLegacy(dir, cb);
      }
      return cb(err);
    });
  } catch (e) {
